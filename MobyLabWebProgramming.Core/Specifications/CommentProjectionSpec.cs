...
public sealed class CommentProjectionSpec : Specification<Comment, CommentDTO>
{
    ...
    /// </summary>
    public CommentProjectionSpec(bool orderByCreatedAt = false, bool textOnly = false)
    {
        if (textOnly)
        {
            Query.Select(e => new()
    ...
    public CommentProjectionSpec(Guid id): this() => Query.Where(e => e.Id == id);

    public CommentProjectionSpec(TorrentDTO torrent) : this(true, true)
    {
        Query.Where(e => e.TorrentId == torrent.Id);
    }
}
