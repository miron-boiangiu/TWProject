...
public class TorrentController(IUserService userService, ITorrentService torrentService) : AuthorizedController(userService)
{
    ...
    public async Task<ActionResult<RequestResponse<PagedResponse<TorrentDTO>>>> GetPage([FromQuery] PaginationSearchQueryParams pagination)
    {
        var currentUser = await GetCurrentUser();

        return currentUser.Result != null ?
            FromServiceResponse(await torrentService.GetTorrentFiles(pagination)) :
            ErrorMessageResult<PagedResponse<TorrentDTO>>(currentUser.Error);
    }
    ...
    public async Task<ActionResult<RequestResponse>> Add([FromForm] TorrentAddDTO form) // The FromForm attribute will bind each field from the form request to the properties of the UserFileAddDTO parameter.
                                                                                         // For files the property should be IFormFile or IFormFileCollection.
    {
        var currentUser = await GetCurrentUser();

        return currentUser.Result != null ?
            FromServiceResponse(await torrentService.SaveTorrentFile(form, currentUser.Result)) :
            ErrorMessageResult(currentUser.Error);
    }

    [Authorize]
    [HttpGet("{id:guid}")] // This attribute will make the controller respond to an HTTP DELETE request on the route /api/User/Delete/<some_guid>.
    public async Task<ActionResult<RequestResponse<TorrentDTO>>> Get([FromRoute] Guid id) // The FromRoute attribute will bind the id from the route to this parameter.
    {
        var currentUser = await GetCurrentUser();

        if (currentUser.Result == null) {
            return ErrorMessageResult<TorrentDTO>(currentUser.Error);
        }

        ServiceResponse<TorrentDTO> torrentServiceResponse = await torrentService.GetTorrent(id, currentUser.Result);

        return FromServiceResponse(torrentServiceResponse);
    }
    ...
    public async Task<ActionResult<RequestResponse>> Download([FromRoute] Guid id)
    {
        ...
        var file = await torrentService.GetTorrentFileDownload(id);
    ...
    public async Task<ActionResult<RequestResponse>> Delete([FromRoute] Guid id) // The FromRoute attribute will bind the id from the route to this parameter.
    {
        ...
        return currentUser.Result != null ?
            FromServiceResponse(await torrentService.DeleteTorrent(id, currentUser.Result)) :
            ErrorMessageResult(currentUser.Error);
    }
}
