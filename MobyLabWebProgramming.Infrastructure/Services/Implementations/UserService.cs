...
/// </summary>
public class UserService(IRepository<WebAppDatabaseContext> repository, ILoginService loginService, IInvitationService invitationService, IMailService mailService)
    : IUserService
{
    ...
    public async Task<ServiceResponse> AddUser(UserAddDTO user, UserDTO? requestingUser = null, Guid? invitationCode = null, CancellationToken cancellationToken = default)
    {
        ...
        if (user.Role == UserRoleEnum.Admin) {
            ...
            {
                return ServiceResponse.FromError(new(HttpStatusCode.Forbidden, "Only admins can add new admins!", ErrorCodes.CannotAdd));
            }
        }
        ...
        if (!allowedToAdd && invitationCode != null && invitationCode is Guid code)
        {
            var invitation = invitationService.UncheckedGet(code);
            if (invitation.Result.Result.Used)
            {
                return ServiceResponse.FromError(new(HttpStatusCode.Conflict, "The invitation code was already used!", ErrorCodes.CannotAdd));
        ...
        if (!allowedToAdd)
        {
            return ServiceResponse.FromError(new(HttpStatusCode.Forbidden, "You require a valid invitation code to register a new user!", ErrorCodes.CannotAdd));
        }
        ...
        if (invitationCode is Guid castedCode)
        {
            await invitationService.UncheckedMarkAsUsed(castedCode, new UserDTO
            {
                Email = newUser.Email,
                Name = newUser.Name,
                Id = newUser.Id,
        ...
        await mailService.SendMail(user.Email, "Welcome!", MailTemplates.UserAddTemplate(user.Name), true, "Torrent app", cancellationToken); // You can send a notification on the user email. Change the email if you want.
