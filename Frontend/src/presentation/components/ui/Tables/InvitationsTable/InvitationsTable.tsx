...
import { useInvitationsTableController } from "./InvitationsTable.controller";
import { InvitationDTO, UserDTO } from "@infrastructure/apis/client";
...
import { GenerateInvitationDialog } from "../../Dialogs/GenerateInvitationDialog";

/**
 * This hook returns a header for the table with translated columns.
 */
const useHeader = (): { key: keyof InvitationDTO, name: string, order: number }[] => {
    const { formatMessage } = useIntl();

    return [
        { key: "id", name: "Invitation Key", order: 1 },
        { key: "createdAt", name: "Generated at", order: 2 }
    ]
};
 ...
 */
const getRowValues = (entries: InvitationDTO[] | null | undefined, orderMap: { [key: string]: number }) =>
    entries?.map(
        entry => {
            return {
                entry: entry,
...
export const InvitationsTable = () => {
    ...
    const { handleChangePage, handleChangePageSize, pagedData, isError, isLoading, tryReload, labelDisplay } = useInvitationsTableController(); // Use the controller hook.

    return <DataLoadingContainer isError={isError} isLoading={isLoading} tryReload={tryReload}> {/* Wrap the table into the loading container because data will be fetched from the backend and is not immediately available.*/}
        <GenerateInvitationDialog /> {/* Add the button to open the user add modal. */}
