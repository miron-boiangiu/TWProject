...
import {ApiTorrentAddPostRequest, Configuration, TorrentApi} from "../client";
...
export const useGetUserFiles = (page: number, pageSize: number, search: string = "") => {
    ...
    return {
        ...useQuery({
            ...
            queryFn: async () => await getFactory(token).apiTorrentGetPageGet({page, pageSize, search}),
...
export const useAddUserFile = () => {
    ...
    return useMutation({
        ...
        mutationFn: async (userFile: ApiTorrentAddPostRequest) => {
            const result = await getFactory(token).apiTorrentAddPost(userFile);
...
export const useDownloadUserFile = () => {
    ...
    return useMutation({
        ...
        mutationFn: async (id: string) => {
            return await getFactory(token).apiTorrentDownloadIdGet({ id });
        }
    })
}